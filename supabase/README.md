...
- immutable workspace-invitation history with recipient consent and owner
  cancellation;
...
- two active channels;
- five representative messages split between the owner and member.
...
tests remain reproducible. Workspace, channel, membership, and message UUIDs
are generated by their command functions and are intentionally not fixtures;
their observable content and relationships form the deterministic seed
contract.
...
| Role             | Stable user ID                         | Email                     | Password       |
| ---------------- | -------------------------------------- | ------------------------- | -------------- |
| Workspace owner  | `10000000-0000-4000-8000-000000000001` | `owner@omoikane.local`    | `Password123!` |
| Workspace member | `10000000-0000-4000-8000-000000000002` | `member@omoikane.local`   | `Password123!` |
| Outsider         | `10000000-0000-4000-8000-000000000003` | `outsider@omoikane.local` | `Password123!` |

The owner and member have active membership in `Omoikane Development`
(`omoikane-development`). The owner has the `owner` role, and the member has the
`member` role. The outsider has an active profile but no membership. The
workspace contains active `General` and `Engineering` channels and five active
messages: three authored by the owner and two by the member.

`tests/database/collaboration_seed_test.sql` protects this observable contract.
Generated aggregate UUIDs and timestamps are excluded so a reset remains free
to recreate the scenario through the production command functions.
